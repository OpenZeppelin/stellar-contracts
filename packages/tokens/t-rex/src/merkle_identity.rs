...
pub trait MerkleIdentityVerifier: IdentityVerifier + ClaimTopics {
    ...
    fn update_claims_root(operator: Address, new_root: [u8; 32]);

    // Updates the Merkle root for revoked claims
    fn update_revoked_root(operator: Address, new_root: [u8; 32]);
    ...
    fn verify_not_revoked(claim_hash: [u8; 32], proof: Vec<[u8; 32]>) -> bool;

    // Gets the current Merkle roots
    fn get_claims_root() -> [u8; 32];
    fn get_revoked_root() -> [u8; 32];

    // Utility to hash a claim for inclusion in the Merkle tree
    fn hash_claim(subject: Address, topic: u32, data: Vec<u8>, signature: Vec<u8>) -> [u8; 32];
}
