...
///
/// The `NonFungibleMintable` trait extends the `NonFungibleToken` trait to provide
/// the capability to mint tokens. This trait is designed to be used in
/// conjunction with the `NonFungibleToken` trait.
///
/// Excluding the `mint` functionality from the [`crate::non_fungible::NonFungibleToken`] trait
...
pub trait NonFungibleMintable {
    ...
    /// # Errors
    ///
    /// * [`crate::NonFungibleTokenError::TokenAlreadyExists`] -
    ///   When the to be minted `token_id` already exists.
    ///
    /// # Events
    ///
    ...
    ///
    /// ```rust
    /// fn mint(&self, e: &Env, to: Address, token_id: u128) {
...
/// * topics - `["mint", to: Address]`
/// * data - `[token_id: u128]`
pub fn emit_mint(e: &Env, to: &Address, token_id: u128) {
