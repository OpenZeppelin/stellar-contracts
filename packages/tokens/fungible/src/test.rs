...
use crate::{
    ...
    storage::{
        ...
        transfer_from, update, AllowanceData, AllowanceKey, StorageKey,
    },
};
...
/// Initialize token with spendable tokens for testing
#[allow(dead_code)]
...
fn test_allowance_structs() {
    ...
    e.as_contract(&address, || {
        ...
        let recreated_key = AllowanceKey { owner: owner.clone(), spender: spender.clone() };

        // Create a key with different addresses
        let diff_owner = Address::generate(&e);
        let different_key = AllowanceKey { owner: diff_owner, spender: spender.clone() };
        ...
        let edge_data = AllowanceData { amount: i128::MAX, live_until_ledger: u32::MAX };
...
fn test_set_allowance_positive_amount_with_expired_ledger() {
    ...
    e.as_contract(&address, || {
        ...
        // This should trigger the specific branch: (amount > 0 && live_until_ledger <
        // current_ledger)
