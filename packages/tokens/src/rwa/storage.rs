use soroban_sdk::{contracttype, panic_with_error, Address, Env, MuxedAddress, String, Vec};
...
impl RWA {
    ...
    ///
    /// * `e` - Access to the Soroban environment.
    /// * `left` - Length of the first array.
    /// * `right` - Length of the array that must match it.
    ///
    /// # Errors
    ///
    /// * [`RWAError::BatchSizeMismatch`] - When the two lengths differ.
    fn require_equal_lengths(e: &Env, left: u32, right: u32) {
        if left != right {
            panic_with_error!(e, RWAError::BatchSizeMismatch);
        }
    }
}
