...
///
/// The balance mirror is authoritative: a wallet can spend at most its
/// mirrored balance minus its still-locked amount. For migration scenarios
/// — registering this module on a token whose holders already own balances
/// — the trait exposes a preset phase. The operator seeds each holder's
/// mirror (and any pre-existing locks) via
/// [`InitialLockupPeriod::preset_lockup_state`], then finalizes the phase
/// with [`InitialLockupPeriod::mark_preset_completed`]. Holders left
/// unseeded have a mirror of zero and cannot send until seeded.
///
