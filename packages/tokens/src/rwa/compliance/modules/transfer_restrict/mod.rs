...
//! Mints are not restricted by this module, mirroring the upstream
//! exemption for zero-address participants.
//!
...
///
/// Mints are intentionally not restricted: the upstream Solidity module
/// exempts operations involving the zero address (mints and burns), and
/// this port mirrors that by always allowing the
/// [`ComplianceModule::can_create`] hook. Burns are likewise unaffected
/// because no validation hook runs for them. The transfer amount has no
