...
use crate::confidential::{
    ...
    wrapper::{
        ...
        emit_set_operator, emit_transfer, emit_withdraw, ConfidentialAccount, OperatorDelegation,
        OperatorTransferPayload, RegisterPayload, RevokeOperatorPayload, SetOperatorPayload,
        TransferPayload, WithdrawPayload, WrapperError, ACCOUNT_EXTEND_AMOUNT,
...
pub enum WrapperStorageKey {
    ...
    /// passed (DESIGN §6.2 *Single-slot semantics*).
...
/// = 35 bytes, base32-encoded to 56 ASCII characters (DESIGN §2.7).
const STRKEY_LEN: usize = 56;

/// 28 bytes per limb in the `address_to_field` decomposition (DESIGN §2.7).
...
/// entries (DESIGN §11.3). Extends the entry's persistent TTL on a hit.
///
/// # Arguments
///
/// * `e` - Access to the Soroban environment.
...
/// without applying the expiry filter (DESIGN §6.2 *Single-slot semantics*).
///
/// # Arguments
///
/// * `e` - Access to the Soroban environment.
...
///
/// Performs the full operation: authorizes `account`, decodes the payload,
/// fetches the auditor key, verifies the proof, persists the new account
/// slot, and emits a `Register` event.
///
/// # Arguments
///
/// * `e` - Access to the Soroban environment.
...
///   registered.
/// * [`WrapperError::InvalidDataPayload`] - When `data` is not a valid
///   [`RegisterPayload`].
...
/// * data - `[auditor_id: u32]`
pub fn register(e: &Env, account: &Address, auditor_id: u32, data: &Bytes) {
    account.require_auth();

    let payload = decode_payload::<RegisterPayload>(e, data);
    ...
    verify(e, CircuitType::Register, &pi, &payload.proof);
    ...
    e.storage().persistent().set(
        &key,
        &ConfidentialAccount {
            spending_key: payload.y,
            viewing_public_key: payload.pvk,
...
/// `to`'s confidential receiving balance (DESIGN §7.3).
///
...
/// resets the receiving slot to the identity (DESIGN §7.4).
///
/// # Arguments
///
/// * `e` - Access to the Soroban environment.
...
pub fn withdraw(e: &Env, from: &Address, to: &Address, amount: i128, data: &Bytes) {
    from.require_auth();
    if amount < 0 {
        panic_with_error!(e, WrapperError::NegativeAmount);
    }

    let payload = decode_payload::<WithdrawPayload>(e, data);
    ...
    verify(e, CircuitType::Withdraw, &pi, &payload.proof);
...
pub fn confidential_transfer(e: &Env, from: &Address, to: &Address, data: &Bytes) {
    ...
    let payload = decode_payload::<TransferPayload>(e, data);
    ...
    verify(e, CircuitType::Transfer, &pi, &payload.proof);
...
/// confidentially to `to` (DESIGN §7.8).
///
/// # Arguments
///
/// * `e` - Access to the Soroban environment.
...
pub fn confidential_transfer_from(
    ...
    let payload = decode_payload::<OperatorTransferPayload>(e, data);
    ...
    verify(e, CircuitType::OperatorTransfer, &pi, &payload.proof);
...
pub fn set_operator(
    ...
    let payload = decode_payload::<SetOperatorPayload>(e, data);
    ...
    verify(e, CircuitType::SetOperator, &pi, &payload.proof);
...
/// escrowed allowance back into `account`'s spendable balance (DESIGN §7.9).
...
pub fn revoke_operator(e: &Env, account: &Address, operator: &Address, data: &Bytes) {
    account.require_auth();

    let payload = decode_payload::<RevokeOperatorPayload>(e, data);
    ...
    verify(e, CircuitType::RevokeOperator, &pi, &payload.proof);
...
///
/// `wrap` is the Poseidon2 compression of the wrapper's own address (DESIGN
/// §2.7, §3.5). It is bound into every owner-initiated circuit's viewing-key
/// derivation, so it must be computed once over
/// `env.current_contract_address()` at construction and never re-derived.
///
/// # Arguments
///
/// * `e` - Access to the Soroban environment.
...
/// as a canonical 32-byte big-endian `Bn254Fr` representative (DESIGN §2.7).
///
...
///   slot is already occupied (DESIGN §6.2 *Single-slot semantics*).
fn set_delegation(
    e: &Env,
    owner: &Address,
    operator: &Address,
...
/// salt after an operator transfer (DESIGN §7.8).
///
/// # Arguments
///
/// * `e` - Access to the Soroban environment.
...
/// Decodes a `data: Bytes` payload as the corresponding XDR-encoded
/// `#[contracttype]` struct.
fn decode_payload<T>(e: &Env, data: &Bytes) -> T
where
    T: FromXdr,
{
    T::from_xdr(e, data).unwrap_or_else(|_| panic_with_error!(e, WrapperError::InvalidDataPayload))
}
...
/// passes the lower or upper 28 bytes of the 56-byte strkey (DESIGN §2.7);
...
fn u256_to_bytes_n_32(e: &Env, u: &U256) -> BytesN<32> {
    BytesN::<32>::try_from(u.to_be_bytes())
        .unwrap_or_else(|_| panic_with_error!(e, WrapperError::InvalidDataPayload))
}
