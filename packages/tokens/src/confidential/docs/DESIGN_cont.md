...
- A retired key's *holder* does not lose what it already opened. The proofless folds -- merge (Section 7.4) and `revoke_spender` (Section 7.9) -- recompute the folded commitment publicly and emit no encrypted checkpoint, so a holder of an already-known opening advances it by the same addition the contract performs. Containment is **event-scoped, not state-scoped**: rotation bounds what a key can newly decrypt, not what its holder can still compute.
...
- **Auditor**: processes events containing $$R\_e$$ to compute ECDH shared secrets and decrypt amounts, balance checkpoints, and the escrowed blindings (Sections 8.1, 8.2, 8.5); folds `Merge`, `RevokeSpender`, and `Clawback` into its accumulators as Section 8.1 specifies.
