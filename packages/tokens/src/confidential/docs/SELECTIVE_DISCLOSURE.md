...
**Distinguishing from D-auditor balance variant (§8).** §8's balance variant decrypts the sender's *post-transfer* balance, and with its lane-2 sibling the full opening, from a specific transfer event — event-anchored, historical, requires auditor cooperation. D-balance is holder-side, reflects *current* on-chain state, and supports predicate-only disclosure that §8's variant does not. The difference is no longer one of capability: the auditor tracks the current opening $$(v\_s, r\_s)$$ of every scoped account's $$C\_{\text{spend}}$$ forward through checkpoints, merges, revokes, and clawbacks (DESIGN_cont.md §8.1, §8.2), so it could satisfy DB3 against the live commitment as readily as the holder. What distinguishes D-balance is who is bound: D1 and D2 tie the proof to the holder's $$sk\_A$$, making it the holder's own attestation, whereas an auditor-produced current-balance proof would bind through auditor key ownership (A1) instead. No such variant is specified here; a recipient that needs a backstop with auditor attestation uses D-auditor over a recent event, and a recipient that needs predicate-only disclosure, or a statement the holder rather than the auditor stands behind, uses D-balance.
