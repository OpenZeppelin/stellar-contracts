...
**Proposition 1** (Merge correctness). <em>If $$C\_{\text{spend}} = \text{Com}(v\_s, r\_s)$$ and $$C\_{\text{receive}} = \text{Com}(v\_r, r\_r)$$ before merge, then after merge $$C\_{\text{spend}} = \text{Com}(v\_s + v\_r, r\_s + r\_r)$$ and $$C\_{\text{receive}} = \mathcal{O} = \text{Com}(0, 0)$$.</em>
...
1. Owner picks ephemeral $$r\_e$$ (reused from the `set_spender` proof's outer ECDH; see §5.3 <em>Why reusing $$r\_e$$ is safe</em>) and computes $$R = r\_e \cdot H$$.
