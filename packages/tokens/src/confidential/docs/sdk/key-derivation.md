...
| $$\text{root}$$ | Exactly one of — the 64-byte ed25519 signature of [Signer roots](#signer-roots), or a raw 32-byte value ([Raw roots and imported keys](#raw-roots-and-imported-keys)) |
| $$\text{addr\\\_f}$$ | $$\text{address\\\_to\\\_field}$$ of the confidential token contract ([Address compression](crypto-core.md#address-compression)) |
...
**Bound to $$\text{addr\\\_f}$$.** $$vk$$ is already deployment-scoped by [Viewing Key](../protocol/keys-and-commitments.md#viewing-key), which bounds the blast radius of a *viewing*-key compromise to one deployment. Binding $$sk$$ likewise bounds a *spending*-key compromise, and the contract address is known whenever a client talks to a deployment.
...
**Availability is not guaranteed.** A signer root exists only where the custody stack implements SEP-0053 message signing, and support across Stellar wallets and hardware apps is uneven. An implementation MUST treat its absence as an expected outcome and fall back to a raw root ([Raw roots and imported keys](#raw-roots-and-imported-keys)), which is not reproducible from anything the user already holds and therefore MUST be backed up explicitly, rather than fail enrolment.
...
- **Contract addresses.** A confidential account registered by a smart account or other contract address has no ed25519 signer to sign [Signer roots](#signer-roots)'s message, so its root comes from whatever custody mechanism controls the contract. Where a smart account authorises through signers of its own, the root is supplied by that mechanism rather than derived from any one of them, since the set can change without the address changing.
- **No SEP-0053 path.** An ed25519-controlled account whose custody stack cannot sign arbitrary messages ([Signer roots](#signer-roots)) falls back here.
- **Imported keys.** Deployments predating this specification hold $$sk$$ values sampled directly from a CSPRNG with no root behind them. Such a key MUST remain usable as a first-class account secret via direct import, bypassing [Derivation](#derivation) entirely.
