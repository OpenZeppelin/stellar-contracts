...
1. D1, D2 force the prover to know an $$sk\_A$$ whose derived $$vk\_A$$ matches the on-chain $$\text{PVK}\_A$$. By [Viewing Key](../protocol/keys-and-commitments.md#viewing-key) this party is the account owner.
2. D3, D4 force $$v\_{\text{transfer}}$$ to equal the decryption of $$\tilde{v}$$ under that owner's $$vk\_A$$. By [ECDH-Derived Blinding](../protocol/keys-and-commitments.md#ecdh-derived-blinding) this is the same value the on-chain transfer commitment $$C\_{\text{transfer}}$$ commits to, since the transfer circuit enforced consistent ECDH derivation at transfer time.
...
D-sender soundness is symmetric: DS3 forces the prover to know $$r\_e$$ with $$R\_e = r\_e \cdot H$$, which by the transfer circuit's constraint T6 ([Confidential Transfer](../protocol/operations/transfer.md)) was the same $$r\_e$$ used to derive the auditor and recipient ciphertexts. DS4, DS5 reconstruct the decryption from the sender side. Soundness rests on DS3's binding through the event's $$R\_e$$ rather than on how the prover came to hold $$r\_e$$; [D-sender](circuits/d-sender.md)'s recovery step bears on wallet state, not on the soundness of the disclosed amount.
...
1. Recover the event's ephemeral scalar $$r\_e$$ per [ECDH-Derived Blinding](../protocol/keys-and-commitments.md#ecdh-derived-blinding) when constructing a D-sender proof ([D-sender](circuits/d-sender.md)). No per-transfer storage is required: both $$r\_e$$ and $$v\_{\text{transfer}}$$ are recomputed at disclosure time from the wallet's $$vk$$ and the on-chain event.
