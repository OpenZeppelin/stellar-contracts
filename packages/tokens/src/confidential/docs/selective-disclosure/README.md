...
- $$sk_A$$, $$vk_A$$, $$\text{PVK}_A$$: an account's spending key, viewing key, and public viewing key ([Key Hierarchy](../protocol/keys-and-commitments.md#key-hierarchy)).
...
- $$K_{\text{aud,s}}$$, $$K_{\text{aud,r}}$$, $$aud_{sk}$$: the sender-side and recipient-side auditor Grumpkin public keys, and an auditor's secret key ([Per-Transfer Auditor Ciphertexts](../protocol/auditing.md#per-transfer-auditor-ciphertexts), [Auditor Key Management and Rotation](../protocol/auditing.md#auditor-key-management-and-rotation)). Each account selects an `auditor_id` at registration; the same `auditor_id` may resolve to either role depending on the transfer's direction.
- $$(R_e, \sigma, \tilde{v}, \tilde{b}, \tilde{v}_{\text{aud,r}}, \tilde{r}_{\text{aud,r}}, \tilde{v}_{\text{aud,s}}, \tilde{b}_{\text{aud,s}}, \tilde{r}_{\text{aud,s}})$$: per-transfer event fields ([Confidential Transfer](../protocol/operations/transfer.md), [Event Schema](../protocol/interface.md#event-schema)). For `SpenderTransfer` events the recipient/auditor ECDH nonce is $$\sigma_a'$$ in place of $$\sigma$$, and the sender-auditor channel emits $$\tilde{a}_{\text{aud,s}}$$ in place of $$\tilde{b}_{\text{aud,s}}$$, with `lane[2]`'s $$\tilde{r}_{\text{aud,s}}$$ carrying the post-transfer allowance blinding $$r_a'$$ rather than a spendable one ([Spender Transfer](../protocol/operations/spender-transfer.md), [Event Schema](../protocol/interface.md#event-schema)). Throughout this document, the symbol $$\sigma_E$$ refers to the **event ECDH nonce**, equal to $$\sigma$$ for `Transfer` events and to $$\sigma_a'$$ for `SpenderTransfer` events; one circuit handles both families, parameterized by which nonce the disclosing event emitted.
...
Two domain separators are specific to this layer: $$\delta_{\text{disc\\\_bind}}$$, the disclosure-ciphertext domain for aggregate disclosures ([Aggregate Disclosures](circuits/aggregate.md)), and $$\delta_{\text{disc}}$$, which keys the disclosure ciphertext to the recipient ([Disclosure Ciphertext to Recipient](protocol.md#disclosure-ciphertext-to-recipient)). Neither is absorbed in a core circuit. Both take their values from [Domain Separation Constants](../protocol/domain-separators.md), which assigns them as a continuation of its own sequence.

A wallet producing a D-sender proof also uses $$\delta_{\text{eph}}$$ ([Domain Separation Constants](../protocol/domain-separators.md)) to recover the event's ephemeral scalar before proving ([D-sender](circuits/d-sender.md)). That use is off-circuit: no disclosure circuit absorbs the tag.
