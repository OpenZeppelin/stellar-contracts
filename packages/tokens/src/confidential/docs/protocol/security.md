...
*Proof.* A spend proof ([Confidential Transfer](operations/transfer.md)) references $$C\_{\text{spend}}^A$$ as a public input. The contract modifies $$C\_{\text{spend}}^A$$ only through:
...
**Corollary.** There is no counter cap on incoming transfers. An account can receive any number of transfers without any mandatory owner action. The receiving balance is a single point whose committed value $$v\_{\text{receive}}$$ grows by exact integer addition: every increment is a contract-validated deposit amount or a circuit-proven transfer amount in $$[0, 2^{127})$$, and since all value entered through deposits, $$v\_{\text{receive}}$$ stays below the underlying token's supply, itself below $$2^{127}$$. Wrap-around in the scalar field would require more than $$2^{127}$$ incoming transfers ([Integer Embedding and Range Proofs](primitives.md#integer-embedding-and-range-proofs)). The next spend after a merge range-checks $$v\_{\text{spend}} + v\_{\text{receive}} \in [0, 2^{127})$$ in-circuit.
...
**Viewing key compromise.** Since $$vk$$ is contract-specific ([Viewing Key](keys-and-commitments.md#viewing-key)), compromise of one contract's viewing key does not affect the owner's accounts in other deployments. Within the compromised contract, the attacker can: read all spendable balance snapshots (via $$\tilde{b}$$ emitted in spend-boundary events), decrypt all incoming transfer amounts (via ECDH with $$R\_e$$ from events), derive all $$dvk\_i$$ to read spender allowances, and recompute the ephemeral scalar $$r\_e$$ of every transfer the account *originated* under the deterministic derivation of [ECDH-Derived Blinding](keys-and-commitments.md#ecdh-derived-blinding), hence the recipient shared scalar $$s$$, hence a full Pedersen opening $$(v\_{\text{transfer}}, r\_{\text{transfer}})$$ of each such $$C\_{\text{transfer}}$$. The attacker **cannot** authorize any spending operation.

Two properties of that last capability deserve stating plainly. It is **retroactive**: the derivation is deterministic in $$(vk, \sigma)$$ and $$\sigma$$ is published, so a compromise today opens every transfer the account ever originated with a derived $$r\_e$$. A transfer whose $$r\_e$$ was sampled instead stays closed: no function of $$(vk, \sigma)$$ reproduces its ephemeral scalar, so neither $$s$$ nor the opening follows. And it **reaches outside the account's own state**: the commitments it opens were added to *recipients'* `receiving_commitment` values. The transfer *amounts* were already inferable from $$vk$$ alone, by differencing consecutive balance checkpoints and netting the inbound credits and merges between them, so amount visibility is not what changes; the openings are, and an opening is a self-verifying artifact that its holder can hand to any third party. This is why $$vk$$ cannot be treated as a safely shareable read-only credential ([Security Requirements](../sdk/requirements.md#security-requirements)): a counterparty that needs visibility into an account's outbound transfers receives per-event disclosure proofs bound to it and to a nonce ([D-sender](../selective-disclosure/circuits/d-sender.md)).

**Auditor key compromise.** An account binds one key for both channels ([Account Data Model](account-state.md#account-data-model)). For every account that used the compromised key, the attacker acquires: all amounts and balance checkpoints; the standing opening of $$C\_{\text{spend}}$$ from the first post-activation checkpoint, maintained across merges exactly as [Per-Transfer Auditor Ciphertexts](auditing.md#per-transfer-auditor-ciphertexts) describes; the opening of $$C\_{\text{receive}}$$ between merges; and the escrowed allowance blinding $$r\_a$$ from every delegation event it observed, hence the opening of each $$C\_a$$ those events wrote ([Spender Allowance Auditing](auditing.md#spender-allowance-auditing)). It cannot recover viewing keys, read data from before the key was active, or authorize any spending. After key rotation, new operations are protected by the new key.
