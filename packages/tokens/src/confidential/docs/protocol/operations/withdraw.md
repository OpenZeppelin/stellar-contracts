...
| W\_a2 | $$s_{a,s} = \text{ECDH}(r_e, K_{\text{aud,s}})$$ (sender-auditor ECDH shared scalar, [Elliptic Curve Diffie-Hellman](../primitives.md#elliptic-curve-diffie-hellman)) |
| W\_a3 | $$(\cdot, m_b, m_r) = \text{SpongeSqueeze}_3(\delta_{\text{aud\\\_s}}, s_{a,s}, \sigma)$$ (sender-auditor channel masks; `lane[0]`, the amount slot, is unused) |
| W\_a4 | $$\tilde{b}_{\text{aud,s}} = (v - a) + m_b$$ (sender-auditor encrypted balance checkpoint) |
| W\_a5 | $$\tilde{r}_{\text{aud,s}} = r' + m_r$$ (sender-auditor escrow of the new spendable blinding, over W5's $$r'$$) |

## Public inputs (16 fields)

| Input | Notes |
|:---|:---|
| $$C_{\text{spend}}$$ | Loaded from `from.spendable_commitment` |
...
| $$C_{\text{spend}}'$$, $$\sigma$$, $$\tilde{b}$$, $$R_e$$, $$\tilde{b}_{\text{aud,s}}$$, $$\tilde{r}_{\text{aud,s}}$$ | Prover-supplied, in this order; $$C_{\text{spend}}'$$ written to `from.spendable_commitment`, the rest emitted in event |
