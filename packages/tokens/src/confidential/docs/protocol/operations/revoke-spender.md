...
**Why the event carries $$\tilde{a}$$ and $$\sigma\_a$$.** The event publishes the `a_tilde` and `allowance_salt` the fold consumes so the recovery above reads both from this event alone. The fold deletes the delegation entry in the same invocation, so neither field is readable from storage afterwards. `SpenderTransfer` writes its $$\tilde{a}'$$ to storage only and emits the replacement salt $$\sigma\_a'$$ it writes, not the salt it consumed ([Transfer nonce](../account-state.md#transfer-nonce)), so without this event the salt would be recoverable only by scanning back to the most recent `SpenderTransfer`'s `sigma_a_new`. For a delegation revoked without ever being spent from, it would be published nowhere: `SetSpender` emits the owner's spendable salt $$\sigma$$ and no allowance salt.
