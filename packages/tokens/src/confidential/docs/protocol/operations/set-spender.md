...
1. Owner derives ephemeral $$r\_e$$ per [ECDH-Derived Blinding](../keys-and-commitments.md#ecdh-derived-blinding) — the same scalar as the `set_spender` proof's outer ECDH; see [Why reusing `r_e` is safe](../keys-and-commitments.md#why-reusing-r_e-is-safe) — and computes $$R = r\_e \cdot H$$.
