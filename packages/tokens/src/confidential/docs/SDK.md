...
**Clawback witness.** In a deployment that enables seizure (COMPLIANCE.md §5), the clawback witness is the auditor secret key together with the pair of openings the client already maintains — the standing sender-channel opening of $$C_{\text{spend}}$$ and the recipient-channel opening of $$C_{\text{receive}}$$ (COMPLIANCE.md §5.2, §5.3). An implementation MUST verify both against the on-chain commitments before proving, MUST read the account's current `clawback_nonce` into the public inputs (COMPLIANCE.md §5.3), and on a `Clawback` event MUST advance the standing opening by the `Merge` rule and the event's public `amount` (COMPLIANCE.md §5.7).
