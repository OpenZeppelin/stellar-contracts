...
4. **On-chain**: During token operations (transfer/mint), the RWA Token:
   ...
   - IdentityVerifier retrieves required claim topics from ClaimTopicsAndIssuers
   - IdentityVerifier fetches claims from investor's IdentityClaims contract
   - IdentityVerifier calls ClaimIssuer to validate signatures
   - IdentityVerifier checks revocation status
   - IdentityVerifier verifies issuer is trusted via ClaimTopicsAndIssuers
