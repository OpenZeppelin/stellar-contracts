...
// //! offering the same features. The key difference lies in how SEP-41
// compliance //! is achieved. The contract in "contract.rs" accomplishes this
// by implementing //! [`openzeppelin_fungible_token::fungible::FungibleToken`]
// and //! [`openzeppelin_fungible_token::burnable::FungibleBurnable`], whereas
// this //! version directly implements [`soroban_sdk::token::TokenInterface`].
// //!
// //! Ultimately, it is up to the user to choose their preferred approach to
// //! creating a SEP-41 token. We suggest the approach in "contract.rs" for
// better //! organization of the code, consistency and ease of
// inspection/debugging.

// use openzeppelin_fungible_token::{self as fungible,
// mintable::FungibleMintable}; use openzeppelin_pausable::{self as pausable,
// Pausable}; use openzeppelin_pausable_macros::when_not_paused;
// use soroban_sdk::{
...
//     /// `TokenInterface` doesn't require implementing `total_supply()`
// because     /// of the need for backwards compatibility with Stellar classic
// assets.     pub fn total_supply(e: &Env) -> i128 {
...
//         let owner: Address = e.storage().instance().get(&OWNER).expect("owner
// should be set");         if owner != caller {
...
// amount: i128) {         fungible::transfer_from(&e, &spender, &from, &to,
// amount);     }

//     fn approve(e: Env, owner: Address, spender: Address, amount: i128,
// live_until_ledger: u32) {         fungible::approve(&e, &owner, &spender,
...
//         // `ownable::only_owner(&e);`
//         let owner: Address = e.storage().instance().get(&OWNER).expect("owner
// should be set");         owner.require_auth();
