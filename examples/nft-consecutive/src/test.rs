...
fn create_client<'a>(e: &Env, owner: &Address, recipient: &Address) -> ExampleContractClient<'a> {
    let address = e.register(ExampleContract, (owner, recipient, 1000));
...
fn initial_state() {
    ...
    let _client = create_client(&e, &owner, &recipient);
